...
describe('Lucid Business Banking - Airtime & Data Spec', () => {
  ...
  it('Should filter history records when searching by phone number', () => {
    ...
    // We use a .should() callback so Cypress retries the query while the table filters.
    cy.get('table tbody tr, [class*="history"] [class*="row"]').should($rows => {
      expect($rows.length).to.be.greaterThan(0);
      $rows.each((index, row) => {
        expect(row.innerText).to.include(testPhone.replace(/^0/, ''));
      });
    });
  });
